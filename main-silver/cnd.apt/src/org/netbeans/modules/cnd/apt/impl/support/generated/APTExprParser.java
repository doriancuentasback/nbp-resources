// $ANTLR Sun-2.7.7(NoEx) (2006-01-29): "support/aptexpr.g" -> "APTExprParser.java"$
...
public class APTExprParser extends org.netbeans.modules.cnd.antlr.LLkParserNoEx       implements APTExprParserTokenTypes
 {
...
protected APTExprParser(TokenBuffer tokenBuf, int k) {
  super(tokenBuf,k);
  tokenNames = _tokenNames;
}

public APTExprParser(TokenBuffer tokenBuf) {
  this(tokenBuf,1);
}

protected APTExprParser(TokenStream lexer, int k) {
  super(lexer,k);
  tokenNames = _tokenNames;
}

public APTExprParser(TokenStream lexer) {
  this(lexer,1);
}
	...
	public final long  ternCondExpr() {
		long r;
		...
		loop3:  while (true) {
			...
			do {
				if ((LA(1)==OR)) {
					...
					b=andExpr();
					if (matchError) {break loop3;}
					r=toLong(toBoolean(r) || toBoolean(b));
				}
				else {
					break _loop8;
				}
