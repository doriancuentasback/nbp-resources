        ...
        return new AsyncCompletionTask(new AsyncCompletionQuery() {
            ...
            protected void query(CompletionResultSet completionResultSet, Document document, int caretOffset) {
                ...
                for (int i = 0; i < locales.length; i++) {
                    ...
                    if (!country.equals("") && country.startsWith(filter) && Utilities.actionsGlobalContext().lookup(Country.class)!=null) {
                        //Here we include the start offset, so that we'll be able to figure out
                        //the number of characters that we'll need to remove:
                        completionResultSet.addItem(new StandardCompletionItem(country, startOffset, caretOffset, 1));
                    }
                }
